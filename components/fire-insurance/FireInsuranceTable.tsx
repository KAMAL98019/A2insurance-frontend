...
function FireDocCell({ record }: { record: FireInsuranceRecord }) {
    ...
    <>
      ...
      <Popover open={Boolean(anchor)} anchorEl={anchor} onClose={() => setAnchor(null)}
        ...
        <Box sx={{ p: 2, width: 220 }}>
          ...
          <Divider sx={{ mb: 1.5 }} />
          <Box
            onClick={() => { if (!url) return; const open = isPdf ? `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api'}/upload/pdf?url=${encodeURIComponent(url)}` : url; window.open(open, '_blank', 'noopener,noreferrer'); }}
...
export default function FireInsuranceTable({ records, loading, onDelete }: Props) {
  ...
  if (!records.length) return (
    <Box sx={{ py: 8, textAlign: 'center' }}><Typography color="text.secondary">No fire insurance records found.</Typography></Box>
  );
            ...
            return (
              <TableRow key={r.id} hover sx={{ '&:last-child td': { borderBottom: 0 } }}>
                ...
                <TableCell sx={{ maxWidth: 130, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  <Typography variant="caption">{r.businessType ?? '—'}</Typography>
                </TableCell>
                <TableCell sx={{ whiteSpace: 'nowrap', fontWeight: 500 }}>{fmtAmount(r.sumInsured)}</TableCell>
