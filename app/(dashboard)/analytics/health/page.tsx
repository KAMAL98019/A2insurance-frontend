...
export default function HealthAnalyticsPage() {
  ...
  const up30    = stats?.upcomingRenewals ?? 0;

  const kpis = [
    { label: 'Total Policies',      value: total,   color: ACCENT,    icon: <MedicalServicesIcon /> },
    ...
    { label: 'Pending Renewal',     value: pending, color: '#e65100', icon: <PendingActionsIcon /> },
  ...
  return (
    <Box>
      <Breadcrumbs sx={{ mb: 1 }}>
        ...
        <Typography color="text.primary">Health Analytics</Typography>
      </Breadcrumbs>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3, flexWrap: 'wrap', gap: 1 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          <Box>
            <Typography variant="h5" sx={{ fontWeight: 700 }}>Health Insurance Analytics</Typography>
            <Typography variant="body2" color="text.secondary">Overview of all health insurance policies</Typography>
          </Box>
        </Box>
        ...
        <Grid container spacing={3}>
          ...
          <Grid size={{ xs: 12, md: 5 }}>
            <Paper sx={{ p: 2.5, borderRadius: 2, height: '100%' }}>
              ...
              <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 2 }}>{total} total policies</Typography>
              <Divider sx={{ mb: 2.5 }} />
              <SvgDonut size={150} data={[
                { label: 'Active',           value: active,  color: '#2e7d32' },
                { label: 'Expired',          value: expired, color: '#c62828' },
                { label: 'Pending Renewal',  value: pending, color: '#e65100' },
          ...
          <Grid size={{ xs: 12 }}>
            <Paper sx={{ p: 2.5, borderRadius: 2 }}>
              <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap', alignItems: 'center' }}>
                <Button variant="contained" disableElevation size="small" sx={{ bgcolor: ACCENT, '&:hover': { bgcolor: '#b71c1c' } }}
                  component={NextLink} href="/health-records/add">Add Health Policy</Button>
                <Button variant="outlined" size="small" component={NextLink} href="/health-records">View All Records</Button>
                <Button variant="outlined" size="small" color="warning" startIcon={<WarningAmberIcon />}
                  component={NextLink} href="/health-records/expiring">Expiring Policies</Button>
                <Button variant="outlined" size="small" color="error" startIcon={<EventBusyIcon />}
                  component={NextLink} href="/health-records/expired">Expired Policies</Button>
              </Box>
            </Paper>
          </Grid>
        </Grid>
